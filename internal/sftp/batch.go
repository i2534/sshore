...
func NewBatchBackend(r osutil.Runner, emit forward.EmitFunc) *BatchBackend {
	return &BatchBackend{runner: r, emit: emit, active: map[string]bool{}, users: map[string]string{}}
}

// controlPathFor 委托给 sshconn（socket 路径的唯一来源）；user 纳入 key，
// 否则同 host 异 user 会共用 master，用错身份读写远端文件。
func (c *BatchBackend) controlPathFor(host, user string) string {
	return sshconn.ControlPath(host, user)
}

func (c *BatchBackend) rememberUser(host, user string) {
	c.mu.Lock()
	if c.users == nil {
...
func (c *BatchBackend) userFor(host string) string {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.users[host]
}

func (c *BatchBackend) run(host, user string, batch []byte) (osutil.Outcome, error) {
...
func (c *BatchBackend) logEvent(host, level, msg string) {
	if c.emit != nil {
...
func (c *BatchBackend) Connect(host, user string) error {
...
func (c *BatchBackend) Disconnect(host string) error {
...
func (c *BatchBackend) disconnectLocked(host, user string) error {
	cp := c.controlPathFor(host, user)
...
func (c *BatchBackend) Connected(host string) bool {
	c.mu.Lock()
	defer c.mu.Unlock()
	if isWindows {
...
func (c *BatchBackend) buildBatch(op, remote, local string) ([]byte, error) {
	switch op {
	case "ls":
...
func (c *BatchBackend) writeBatch(dir string, content []byte) (string, error) {
...
func (c *BatchBackend) List(host, user, path string) ([]Item, error) {
...
func (c *BatchBackend) ListMany(host, user string, paths []string) (map[string][]Item, error) {
	if len(paths) == 0 {
...
func (c *BatchBackend) Home(host, user string) (string, error) {
...
func (c *BatchBackend) remoteSize(host, user, path string) (int64, error) {
	batch, err := c.buildBatch("ls", path, "")
	if err != nil {
		return -1, err
	}
...
func (c *BatchBackend) Get(host, user, remote, local string) error {
...
func (c *BatchBackend) GetRecursive(host, user, remote, local string) error {
...
func (c *BatchBackend) Put(host, user, local, remote string) error {
...
func (c *BatchBackend) PutRecursive(host, user, local, remoteDir string) error {
...
func (c *BatchBackend) Remove(host, user, path string) error {
...
func (c *BatchBackend) RemoveRecursive(host, user, path string) error {
	if path == "" || path == "/" {
...
func (c *BatchBackend) removeBatch(host, user string, files, dirsBottomUp []string) error {
	var sb strings.Builder
...
func (c *BatchBackend) Mkdir(host, user, path string) error {
...
func (c *BatchBackend) Rename(host, user, oldPath, newPath string) error {
