...
//     再关闭全部会话（中断阻塞中的网络 IO）。强停前绝不清理：只要还有传输可能提交，
//     就绝不删它的 .part。**注意**：强停只能中断网络 IO，卡在不可中断本地写的传输仍可能
//     晚于 CloseAll 收尾（见下方 RecoverSwaps 段），不能把它当作「绝无在飞提交」的保证。
//  3. 关闭全部会话（正常路径：此刻没有在途 IO；强停路径：这是第二次 pool.CloseAll）。
//     强停路径的第二次调用是**幂等加固**：第一次（第 2 步）取池快照之后仍可能有会话逃过
//     该快照（并发 dial 完成、刚被重新登记进池），第二次才把它清掉（Task 13 重审 B2 的
//     窄窗口）。CleanupParts **不在**这条因果里：它在第 4 步才跑，且自己用
//     Release(s, false) 关掉取来的临时会话，不存在「再补一次 CloseAll」这回事。
//  4. 清理已知 .part —— 必须放在关会话**之后**：CleanupParts 自己会为每个 host 取会话
//     （pool.CloseAll 之后仍可 AcquireList 新建）。此时在飞传输要么已结束、要么已被强停；
//     登记表里剩下的 .part 属于已停止的传输（强停路径下极少数卡死的传输是例外，见下）。
//  5. 清连接意图（CleanupParts 取会话会重新点亮 Connected，必须在最后清）。
//
// app.go 的 OnShutdown 在本方法返回后才调用 RecoverSwaps，但两条路径的保证强度不同
// （Task 13 重审 B1；app.go 的对应注释在 Task 14 已同步，两边必须一致）：
//   - **正常路径**（宽限期内全部传输结束）：此刻没有在飞提交，恢复探测绝不会与它竞争。
//   - **强停路径**（宽限期耗尽）：取消 ctx + 关会话只保证能中断阻塞中的**网络 IO**；
//     一个已经卡在不可中断本地写（例如本地磁盘 hang）的传输仍可能在 CloseAll 返回后
//     继续跑完并尝试提交。此时 RecoverSwaps 与该次提交存在真实竞态：恢复探测可能把
//     一次「target→bak 已成功、part→target 尚未」的在飞提交当成 W0 处理。
//     这是「有界退出 vs 绝不与在飞传输竞争」二选一时选前者的代价 —— 退出必须有确定上界，
//     因此接受这个低概率窗口。RecoverSwaps 的收敛规则（W0 只删「两端都不存在/都已回滚
//     干净」的条目，能回滚就回滚）**不是**为了消除该竞态，而是把误判的破坏面尽量压小。
//
// 旧顺序（有传输在飞时先删 .part，或不等静默就恢复）会留下 target 缺失、bak 残存、
// journal 条目被清的现场，这才是修复轮 2 / F2 要避免的问题。
//
...
//
// 强停竞态（Task 13 重审 B1）：CloseAll 的强制停止路径下，卡在不可中断本地写的传输可能
// 晚于 CloseAll 返回并恰好停在 rename(target→bak)/rename(part→target) 之间，本函数于是可能
// 与这次在飞提交竞争。下面的文件系统探测（而不是「见条目就回滚」）正是在这种竞态下把破坏面
// 压到最小的收敛规则，但它**不能消除**竞态 —— 消除它就得放弃有界退出，取舍见 CloseAll 注释。
